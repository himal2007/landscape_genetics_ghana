...
p1 <- p + 
  ...
  geom_sf(data = bui_np_crop, col = "black", fill = "lightgreen", lwd = 0.5, lty = 1, alpha = 0.5)+
...
p3 <- direction + geom_point(data = sample_info, aes(X, Y, shape = samples), size = 3, fill = "red", alpha = .6, colour = "black", stroke = 1) +
  geom_label_repel(data = sample_info, aes(X, Y, label = pop_id), size = 5, alpha = .8) + theme_bw(base_family = "Arial", base_size = 18) + scale_shape_manual(values = c(21,24)) #, shape = 21, colour = "black", fill = "white",  alpha = 0.8) # +
  # facet_wrap(~samples, ncol = 1)


p3$labels$shape <- "Samples" #"Sample size"

ggsave(plot = p3, filename = "figs/220919_sample_locations_vec_par.png", device = "png", dpi = 500, width = 9, height = 5, units = "in" )
```

```{r}
sample_info_parasites <- sample_info %>% dplyr::filter(samples == "parasites")

parasite_only <- direction + geom_point(data = sample_info_parasites, aes(X, Y), shape = 21, size = 3, fill = "red", alpha = .6, colour = "black", stroke = 1) +
  geom_label_repel(data = sample_info_parasites, aes(X, Y, label = pop_id), size = 5, alpha = .8) + theme_bw(base_family = "Arial", base_size = 18)

ggsave(plot = parasite_only, filename = "figs/220919_sample_locations_par_onlt.png", device = "png", dpi = 500, width = 9, height = 5, units = "in" )
```
